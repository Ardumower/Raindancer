...
public:
	...
	// of its Run() function.
	Protothread() : interval(0), last_run(0), _ptLine(0) {}

	// Restart protothread.
	void Restart() { _ptLine = 0; 
			     last_run = millis() - interval;
	}
...
protected:
	...
	// Time variables for PT_YIELD_MS
...
// Yield protothread until given millis are expired.
#define PT_YIELD_MS(ms) \
    do {interval=ms; last_run=millis(); ptYielded = false; _ptLine = __LINE__; case __LINE__: \
    if (!ptYielded || (millis() - last_run < interval)) return true; } while (0)

// Yield protothread until interval is expired.
#define PT_YIELD_INTERVAL() \
    do {last_run=millis(); ptYielded = false; _ptLine = __LINE__; case __LINE__: \
    if (!ptYielded || (millis() - last_run < interval)) return true; } while (0)
